            ...
            <Card className="bg-yellow-50/10 border-yellow-200 mb-4">
              <CardContent className="pt-6">
                <p className="text-sm text-yellow-300">
                  <strong>Grace Period:</strong> {billingStatus.daysUntilExpiry} days remaining
                  {billingStatus.gracePeriodEnds && (
                    <span className="block text-xs mt-1">
                      Expires: {new Date(billingStatus.gracePeriodEnds).toLocaleDateString()}
                    </span>
                  )}
                </p>
              </CardContent>
...
export function SubscriptionBanner({ billingStatus }: SubscriptionBannerProps) {
  ...
  return (
    <div className={`border-l-2 p-3 mb-4 rounded-r-md ${getBannerStyle()}`}>
