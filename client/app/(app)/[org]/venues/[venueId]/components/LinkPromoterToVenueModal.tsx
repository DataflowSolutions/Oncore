...
import { Plus, Search, Loader2, Mail, Phone, Building, MapPin, Users } from 'lucide-react'
...
export function LinkPromoterToVenueModal({ 
  ...
  const handleLink = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        toast.success(
          `Linked ${successCount} promoter${successCount > 1 ? 's' : ''} to ${venueName}`
        )
        router.refresh()
        setOpen(false)
      }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[80vh]">
        <DialogHeader>
          <DialogTitle>Link Promoters to {venueName}</DialogTitle>
          <DialogDescription>
            Select existing promoters to link to this venue. Already linked promoters are hidden.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between pt-4 border-t">
            <p className="text-sm text-muted-foreground">
              {selectedPromoters.size} promoter{selectedPromoters.size !== 1 ? 's' : ''} selected
            </p>
