-- Update promoter link target and ensure function recreation
...
AS $$
  ...
  -- Contacts/Promoters (link to promoters view under venues)
  SELECT 
    c.id,
  ...
  -- Calendar/Schedule Events (with date param)
  SELECT 
    si.id,
    'event'::text as type,
    si.title as title,
    ...
    '/shows/' || si.show_id::text || '/day?date=' || TO_CHAR(si.starts_at, 'YYYY-MM-DD') as url,
    CASE 
