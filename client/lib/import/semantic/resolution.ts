...
export async function resolveImportFacts(
  ...
  const groups = groupFactsForResolution(facts);
  ...
  if (llmResponse.error || !llmResponse.content) {
    ...
    resolutions = parseLLMResolutions(llmResponse.content, facts);
  ...
  resolutions = validateResolutions(resolutions, facts, warnings);
