...
export async function createPerson(formData: FormData) {
  ...
  const { data: membership, error: membershipError } = await (
    supabase as any
  ).rpc("get_org_membership", {
    p_org_id: validatedData.orgId,
  });

  if (membershipError || !membership) {
    throw new Error(
      "You do not have permission to add people to this organization"
    );
  }
  ...
  const personData: PersonInsert = {
    ...
    member_type:
      validatedData.memberType as Database["public"]["Enums"]["member_type"],
  };
...
export async function updatePerson(personId: string, updates: PersonUpdate) {
  ...
  // Use RPC function to update person (bypasses RLS issues with PostgREST)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data, error } = await (supabase as any).rpc("app_update_person", {
    p_person_id: personId,
    p_name: updates.name || null,
    p_email: updates.email || null,
    p_phone: updates.phone || null,
    p_role_title: updates.role_title || null,
  ...
  // Get org_id from the result for revalidation
...
export async function deletePerson(personId: string, orgId: string) {
  ...
  // Use RPC function to delete person (bypasses RLS issues with PostgREST)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { error } = await (supabase as any).rpc("app_delete_person", {
    p_person_id: personId,
  });
...
export async function inviteOrgMember(formData: FormData) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: membership } = await (supabase as any).rpc(
    "get_org_membership",
    {
      p_org_id: validatedData.orgId,
    }
  ...
  logger.debug("Would send invitation", {
