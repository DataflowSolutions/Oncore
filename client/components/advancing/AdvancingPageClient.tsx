...
export function AdvancingPageClient({
  ...
  const { party, setParty, setArtistData, setPromoterData, getCurrentData } =
    useAdvancingStore();
  ...
  const currentPartyDocuments = documents.filter(
    (doc) => doc.party_type === party
  );
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100">
      ...
      {/* Main Content */}
      <div className="bg-card mx-auto px-6 py-6">
        {party === "from_you" ? (
            ...
            {Object.keys(fieldsBySection).length === 0 ? (
              <div className="text-center py-12">
                <p className="text-neutral-400">
                  No fields configured for this session yet.
                </p>
              </div>
            ) : (
              <>
