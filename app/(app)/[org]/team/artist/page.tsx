...
import { Music, Mail, Phone, FileText } from 'lucide-react'

interface ArtistTeamPageProps {
  params: Promise<{ org: string }>
}

export default async function ArtistTeamPage({ params }: ArtistTeamPageProps) {
  ...
  // Get people filtered by artist member type
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-lg">Artist Team ({artistTeam.length})</CardTitle>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          {artistTeam.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">No artist team members added yet</p>
            </div>
          ) : (
