...
export default async function TourLayout({ children, params }: OrgLayoutProps) {
  ...
  if (billingStatus && shouldShowBillingGate(billingStatus) && !resolvedParams.org.includes('billing-debug')) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      {billingStatus && (
        <SubscriptionBanner billingStatus={billingStatus} />
      )}
      
      <div className="pt-10 lg:flex justify-end mb-4 hidden">
        <TabNavigation orgSlug={resolvedParams.org} userRole={membership.role} />
      </div>
